/* 
Server for Assignment 06
Author: Deborah Yuan & Evon Diep & Gage Minamoto
Date: 12/12/22
...
function setDynamicPrice(
    ...
    console.log("Series: " + products_data[series]);
...
function applyDiscountPercentage( // the function for discount percentage
    products_data, // an object containing product data
    series, // a string representing a series of products
    i, // an index representing a specific product within the series
    discount_percent, // a number representing the discount percentage
    requestcheckbox // a boolean representing whether or not a discount is requested
) {
    if (requestcheckbox == undefined) { // check if discount is requested
        // discount was whatever it was before
        console.log("no discount" + products_data[series][i].discount_percentage); // log current discount percentage
        products_data[series][i].discount_percentage =
            products_data[series][i].discount_percentage; // do not change discount percentage
    } else {
        products_data[series][i].discount_percentage = discount_percent; // set discount percentage to requested value
        products_data[series][i].sale_price =
            products_data[series][i].price * ((100 - discount_percent) / 100); // calculate sale price based on discount percentage
        products_data[series][i].dynamic_pricing = false; // set dynamic pricing to false
        console.log("yes discount" + products_data[series][i].discount_percentage); // log new discount percentage
    }
    // return updated product data object
...
app.post("/pricingmodule", function (request, response) {
    ...
    for (i in products_data[series]) {
        ...
        if (
            ...
            // var series = 
            // var i = which iteration of loop you're in; give it a number 1 or 2;
            // var i= 2 woud be third product on iphone list
            // var discount_percent = 99, -99
            // var requestcheckbox = true or undefined
            ...
            // var dynamic_pricing == true
            // var selected = true
            // var series = 'iPhone'
            // currentDate = 5/10/2023 4:50AM NEEDS TO BE A DATE
            // new Date()

        } else if (dynamic_pricing == true) {
            if (
                request.body[`pricingmodule_select_${series}+${i}`] == undefined || // dynamic pricing does not get triggered
    ...
    if (
        ...
        for (series in products_data) {
            ...
            for (i = 0; i < products_data[series].length; i++) {
                console.log(
                    "# of items in " + series + ": " + products_data[series].length
                );
...
app.get("/editaccount", function (request, response) {
	...
	<nav class="navbar navbar-inverse">  
	  <div class="container-fluid">
		...
		<ul class="nav navbar-nav navbar-right">
		<ul class="nav navbar-nav">
		<li class="active"><a href="./loginsuccess"><span class="glyphicon glyphicon-user"></span>&emsp;${users[active_user].fullname
            }&emsp;</a></li>
		</ul>
		<ul class="nav navbar-nav">
		 <li><a href="./cart">&emsp;<span class="glyphicon glyphicon-shopping-cart"></span>&emsp;Cart (${totalItems})&emsp;</a></li>
