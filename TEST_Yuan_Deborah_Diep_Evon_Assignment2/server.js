...
var tempname = __dirname + "/tempinfo.json";
...
if (fs.existsSync(tempname)) {
	...
	temporary = JSON.parse(tempdata);
	console.log(temporary);
} else {
	console.log("Sorry file " + tempname + " does not exist.");
	temporary = {};
}
...
app.post("/purchase", function (request, response) {
	...
	console.log("hi" + customerquantities);
	...
	let user_pass2 = POST["passwordconfirm"];*/
		...
		/*if (users[user_name] == undefined && user_pass == user_pass2) {
			if (users[user_name] != user_name) {
				users[user_name] = {};
				users[user_name].quantities = qtys;
				loggedin = true;
		...
		if (qtys == 0) {
			...
			temporary[products[i].name] = qtys;
				temporary[USERNAME?].model = XXX[products[i].name].quantities = qtys;
			let tempdata = JSON.stringify(temporary);
